...
เว็บแอป Streamlit ภาษาไทยที่ผสาน Cognitive Functions 80 ข้อ, แบบประเมินความสนใจและความถนัด 100 ข้อ, และคำถามทุน/งบประมาณ 1 ข้อ เพื่อเสนอคณะ/สาขาที่ผ่านกฎตรรกศาสตร์เริ่มต้น
...
- รวมคะแนนความถนัด 5 หมวด (`M`, `S`, `L`, `H`, `A`) หมวดละ 20 ข้อ เป็นคะแนนเต็ม 100 และเปอร์เซ็นต์
- ใช้ประพจน์ `(MBTI₁ ∨ …) ∧ (หมวด > เกณฑ์%)` คัดเลือกคณะ/สาขา
- เชื่อมระดับค่าเรียนของคณะกับเงินทุนของผู้เรียน — คำถามทุน/งบประมาณอยู่ท้ายหน้าสรุป และคณะที่ระดับค่าเรียนสูงกว่างบจะไม่ถูกแนะนำ (เช่น แพทยศาสตร์ต้องการงบมาก)
- กรองตัวอย่างเส้นทางมหาวิทยาลัยตามงบ: น้อย / ปานกลาง / มาก
- ดาวน์โหลดรายงานผลเป็น JSON
- บันทึกผลลง SQLite และดูประวัติผลลัพธ์ร่วมกันได้ พร้อมสิทธิ์ลบสำหรับผู้ดูแล
...
## ประวัติผลลัพธ์และผู้ดูแล

ผลที่ผู้ใช้กดบันทึกจะเก็บในไฟล์ results.db และทุกคนสามารถดูได้จากเมนู **ประวัติผลลัพธ์** ผู้ดูแลลบรายการได้เมื่อกำหนดรหัสผ่านไว้ก่อน โดยตั้งค่าตัวแปรแวดล้อมหรือ Streamlit secrets ดังนี้:

```toml
# .streamlit/secrets.toml
UNIMATCH_ADMIN_PASSWORD = "เปลี่ยนเป็นรหัสผ่านที่ปลอดภัย"
```

อย่า commit รหัสผ่านหรือไฟล์ results.db ที่มีข้อมูลผู้ใช้งานขึ้น GitHub. สำหรับ Streamlit Community Cloud ไฟล์ SQLite ในเครื่องอาจไม่คงอยู่หลัง restart/redeploy; หากต้องการประวัติที่ใช้ร่วมกันแบบถาวร ควรเปลี่ยนไปใช้ฐานข้อมูลภายนอก.
