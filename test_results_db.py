...
def test_save_list_and_delete_result(tmp_path):
    ...
    first_id = save_result("คนแรก", "INTP", "วิทยาการคอมพิวเตอร์", 91, "งบน้อย", "ผ่านตรรกะ strict", database)
    second_id = save_result("คนที่สอง", "INFJ", "จิตวิทยา", 82, "งบปานกลาง", "ผ่อนเกณฑ์วิชา 10%", database)
    ...
    assert records[0]["selection_basis"] == "ผ่อนเกณฑ์วิชา 10%"
...
def test_result_name_and_compatibility_are_validated(tmp_path):
    ...
    try:
        save_result("  ", "INTP", "คณะทดสอบ", 70, None, "ผ่านตรรกะ strict", database)
        assert False, "expected empty name to fail"
    except ValueError:
        pass

    try:
        save_result("ทดสอบ", "INTP", "คณะทดสอบ", 101, None, "ผ่านตรรกะ strict", database)
